 ...
 * 
 */
public class WaterCycleInputHandler extends InputHandler {
    ...
    private static class SingletonHolder {
        public static final WaterCycleInputHandler INSTANCE = new WaterCycleInputHandler();
    }
     ...
     */
    public static WaterCycleInputHandler getInstance() {
        return SingletonHolder.INSTANCE;
    }

    protected WaterCycleInputHandler() {
        reset();
    }
    ...
    public void mouseWheelMoved(MouseEvent e) {
        ...
        float minDist = -50f;
        float diff = viewDist - minDist;

        if (e.isShiftDown()) {
            float wheelRotation = e.getRotation()[0];
            newViewDist -= wheelRotation * (diff * .01);
        } else {
            float wheelRotation = e.getRotation()[1];
            newViewDist -= wheelRotation * (diff * .1);
        }
